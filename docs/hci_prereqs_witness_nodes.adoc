...
기존 Witness Node를 Element 12.8 이상으로 인플레이스 업그레이드하는 경우, 업그레이드하기 전에 Witness Node의 VM 하드웨어 버전이 환경에 따라 ESXi 6.7(VM 하드웨어 버전 14) 이상과 호환되는지 확인해야 합니다. 보다link:task_hcc_upgrade_management_node.html#upgrade-vm-hardware["VM 하드웨어 버전을 업그레이드합니다"] .
