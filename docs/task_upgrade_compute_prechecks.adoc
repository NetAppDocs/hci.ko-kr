...
.무엇을 &#8217;필요로 할거야
* 최신 관리 서비스 번들(2.11 이상)으로 업데이트되었습니다.
...
. 상태 점검을 클릭합니다 image:hcc_healthcheck_icon.png["아이콘을 클릭합니다"] 업그레이드 준비 상태를 확인하려는 클러스터의 경우
. Compute Health Check * 페이지에서 * Run Health Check * 를 클릭합니다.
. 문제가 있는 경우 페이지에 보고서가 제공됩니다. 다음을 수행합니다.
+
.. 각 문제에 대해 나열된 특정 KB 문서로 이동하거나 지정된 해결책을 수행합니다.
.. KB가 지정된 경우 관련 KB 문서에 설명된 프로세스를 완료합니다.
.. 클러스터 문제를 해결한 후 * 상태 점검 다시 실행 * 을 클릭합니다.
...
.. REST API UI에서 * get si./installationsi./{id} * 를 클릭합니다.
.. 체험하기 * 를 클릭합니다.
.. 설치 ID를 입력합니다.
.. Execute * 를 클릭합니다.
...
.. POST/computeeh./{controller_ID} hifx/health-checks * 를 클릭합니다.
.. 체험하기 * 를 클릭합니다.
...
----
.. 클러스터에서 상태 점검을 실행하려면 * 실행 * 을 클릭합니다.
+
...
https://<ManagementNodeIP>/vcenter/1/
----
.. get/computeh./tasks/{task_id} * 를 클릭합니다.
.. 체험하기 * 를 클릭합니다.
