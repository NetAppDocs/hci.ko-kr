...
.단계
. 로 이동합니다 https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab["NetApp HCI 소프트웨어 다운로드"] 페이지를 방문하여 올바른 버전의 NetApp HCI에 대한 다운로드 링크를 클릭합니다.
...
. Remediate * 를 클릭하고 새로 생성된 호스트 기준선을 선택합니다. 기준에 포함된 드라이버가 선택되었는지 확인합니다.
...
. 마법사의 * Ready to Complete * 페이지로 이동한 후 * Finish * 를 클릭합니다.
+
